    ...
    sendData('forgotPasswordRequest', data).then((response)=>{
      if(response.message==="invalid_request"){
        ...
        if(response.message==="success"){
          ...
          toast_options.message = messageGenerator({initiator:"passforgot", message_type:response.message});
          toast_options.timer=16000;
        }else if (response.message==="noaccount") {
          toast_options.message = messageGenerator({initiator:"passforgot", message_type:response.message, message_part:`an account for the given information!</b><br/>`});
        }else{
...
/*
* CREATE HTML FOR FORGOT PASSWORD INTERFACE
*/
